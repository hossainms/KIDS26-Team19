...
# Fallback: fetch ONLY the series title for a GSE via GEOquery, used when the
# local matrix-file parse fails to recover a title (e.g. some multi-platform
# files). GSEMatrix = FALSE + getGPL = FALSE keeps this to the lightweight SOFT
# metadata, so it does not re-download the expression matrix or platform table.
.geo_query_title <- function(gse) {
  if (!requireNamespace("GEOquery", quietly = TRUE)) {
    stop("Title fallback requires GEOquery. Install with BiocManager::install('GEOquery').")
  }
  object <- GEOquery::getGEO(gse, GSEMatrix = FALSE, getGPL = FALSE)
  title <- GEOquery::Meta(object)$title
  if (is.null(title) || !length(title)) NA_character_ else paste(title, collapse = "; ")
}
...
run_geo_pipeline <- function(input = "Pediatric_AML.xlsx",
                             input_sheet = "Pediatric_AML_Studies",
                             out_dir = "downloads/geo_metadata_inventory",
                             query_strings = NULL, keywords = character(),
                             max_studies_per_query = Inf, retries = 1L,
                             limit = Inf,
                             use_geoquery_title_fallback = TRUE) {
  ...
  if (is.null(query_strings)) {
    ...
    metadata <- as.data.frame(readxl::read_excel(input, sheet = input_sheet))
  ...
  # Normalize column names from the pediatric workbook to the names the
  # metadata-merge step below expects. The pediatric sheet uses "Title" and
  # "PubMedIDs"; older inventories used "SeriesTitle" and "PubMedID". These are
  # only a fallback anyway (title/organism/sample count are re-parsed from each
  # downloaded matrix file), but aligning the names keeps the fallback working.
  if (!("SeriesTitle" %in% names(metadata)) && ("Title" %in% names(metadata))) {
    metadata$SeriesTitle <- metadata$Title
  }
  if (!("PubMedID" %in% names(metadata)) && ("PubMedIDs" %in% names(metadata))) {
    metadata$PubMedID <- metadata$PubMedIDs
  }
  if (!("SampleCount" %in% names(metadata)) && ("TotalSamples" %in% names(metadata))) {
    metadata$SampleCount <- metadata$TotalSamples
  }
  metadata$SeriesAccession <- toupper(trimws(as.character(metadata$SeriesAccession)))
  ids <- unique(metadata$SeriesAccession)
  # TEST-RUN OPTION: process only the first `limit` studies. Leave as Inf for a
  # full run; set to e.g. 5 to sanity-check output before committing to the
  # whole batch.
  if (is.finite(limit)) {
    ids <- head(ids, as.integer(limit))
    message("limit set: processing only the first ", length(ids), " studies.")
  }
  processed <- 0L
  ...
  for (i in seq_along(ids)) {
    ...
    for (j in seq_len(nrow(found))) {
      ...
      if (row$Status %in% c("downloaded", "already_exists")) {
        ...
        # FIX: a field can come back as a multi-element vector (common for
        # multi-platform files). The old `if (!is.na(details[[field]]))` errored
        # with "condition has length > 1" on those rows, aborting the whole
        # metadata merge and leaving SeriesTitle blank. Collapse safely instead.
        for (field in names(details)) {
          value <- details[[field]]
          value <- value[!is.na(value) & nzchar(trimws(as.character(value)))]
          if (length(value)) row[[field]] <- paste(unique(value), collapse = "; ")
        }
      }
      # FALLBACK: if the title is still missing after the local parse, fetch it
      # from NCBI via GEOquery (lightweight SOFT metadata). Only fires when
      # needed, so it adds network calls only for the problem rows.
      if (isTRUE(use_geoquery_title_fallback) &&
          row$Status %in% c("downloaded", "already_exists") &&
          (is.null(row$SeriesTitle) || is.na(row$SeriesTitle) || !nzchar(trimws(row$SeriesTitle)))) {
        fetched_title <- tryCatch(.geo_query_title(row$SeriesAccession),
                                  error = function(e) NA_character_)
        if (!is.na(fetched_title) && nzchar(trimws(fetched_title))) {
          row$SeriesTitle <- fetched_title
        }
        Sys.sleep(0.4)  # be polite to NCBI
      }
...
.geo_main <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (length(args) && args[1] == "--help") {
    cat("Workbook: Rscript 05_Download_Metadata_Inventory.R [input.xlsx] [output_directory]\n",
        "          (defaults to Pediatric_AML.xlsx, sheet 'Pediatric_AML_Studies')\n",
        ...
        "Test run (first N studies): add --limit 5 to either mode\n",
        "Disable GEOquery title fallback: add --no-title-fallback\n")
    return(invisible(NULL))
  }
  # Pull out the standalone flag before parsing key/value options.
  use_title_fallback <- !("--no-title-fallback" %in% args)
  args <- args[args != "--no-title-fallback"]

  if (length(args) && args[1] == "--query") {
    if (length(args) %% 2L != 0L || any(!args[seq(1L, length(args), 2L)] %in%
                                        c("--query", "--out", "--keywords", "--max-studies", "--limit"))) stop("Invalid arguments; use --help.")
    ...
    run_geo_pipeline(query_strings = options[["--query"]],
                     ...
                     limit = as.numeric(get_option("--limit", "Inf")),
                     use_geoquery_title_fallback = use_title_fallback)
  } else {
    # Positional args are [input.xlsx] [output_directory]; an optional
    # "--limit N" pair may follow them.
    limit_val <- Inf
    limit_pos <- which(args == "--limit")
    if (length(limit_pos) == 1L && length(args) >= limit_pos + 1L) {
      limit_val <- as.numeric(args[limit_pos + 1L])
      args <- args[-c(limit_pos, limit_pos + 1L)]
    }
    if (length(args) > 2L) stop("Invalid arguments; use --help.")
    run_geo_pipeline(input = if (length(args)) args[1] else "Pediatric_AML.xlsx",
